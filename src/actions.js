...
const BEDROCK_GUIDE = [
  ...
  "",
  "EVERY command Bedrock has. Nothing outside this list exists. If a request",
  "needs a command that is not here, say so plainly instead of inventing one.",
  "",
  "These are Java only and DO NOT EXIST in Bedrock, never write them:",
  "  data, item, attribute, team, advancement, bossbar, datapack, worldborder,",
  "  forceload, spectate, jfr, perf, random, return, rotate, tick, trigger,",
  "  place template, execute store, execute if, execute unless",
  "",
  "Syntax of the ones worth using:",
  "  ability <player> <ability> [true|false]",
  "  camerashake add <player> [intensity] [seconds] [positional|rotational]",
  "  camera <player> set <preset> / camera <player> fade [time <in> <hold> <out>]",
  "  clear [player] [item] [data] [maxCount]",
  "  clone <x1 y1 z1> <x2 y2 z2> <dest> [masked|replace] [normal|force|move]",
  "  damage <target> <amount> [cause] [entity <damager>]",
  "  daylock <true|false>",
  "  dialogue open <npc> <player> [scene] / dialogue change <npc> <scene>",
  "  difficulty <peaceful|easy|normal|hard>",
  "  effect <target> <effect> [seconds] [amplifier] [hideParticles]",
  "  effect <target> clear",
  "  enchant <player> <enchantment> [level]",
  "  event entity <target> <event>",
  "  execute as <sel> at <sel> [positioned <x y z>] run <command>",
  "  fill <from> <to> <block> [replace|hollow|outline|keep|destroy] [oldBlock]",
  "  fog <player> push <fogId> <name> / fog <player> pop <name>",
  "  function <path>",
  "  gamemode <survival|creative|adventure|spectator> [player]",
  "  gamerule <rule> [value]",
  "  give <player> <item> [amount] [data]",
  "  hud <player> <visible|hidden> [element]",
  "  inputpermission set <player> <permission> <enabled|disabled>",
  "  kill [target]",
  "  locate biome <biome> / locate structure <structure> [useNewChunks]",
  "  loot spawn <x y z> loot <table> / loot give <player> loot <table>",
  "  me <message> / say <message> / tell <player> <message>",
  "  mobevent <event> [true|false]",
  "  music play|queue|stop|volume <track> [volume] [fade] [loop]",
  "  particle <name> <x y z>",
  "  place feature|featurerule|jigsaw|structure <name> <pos>",
  "  playanimation <target> <animation> [next] [blend] [stop] [controller]",
  "  playsound <sound> [player] [x y z] [volume] [pitch] [minVolume]",
  "  recipe give|take <player> <recipe|*>",
  "  replaceitem block <x y z> slot.container <slot> <item> [amount] [data]",
  "  replaceitem entity <target> <slot> <slotId> <item> [amount] [data]",
  "  ride <riders> start_riding <ride> [teleport] [how]",
  "  ride <rides> evict_riders / ride <riders> stop_riding",
  "  ride <target> summon_rider|summon_ride <entity> [rule] [event] [name]",
  "  schedule on_area_loaded add <from> <to> <function>",
  "  scoreboard objectives add|remove|list|setdisplay <...>",
  "  scoreboard players add|set|remove|list|reset|operation|random|test <...>",
  "  setblock <x y z> <block> [destroy|keep|replace]",
  "  setworldspawn [x y z] / spawnpoint [player] [x y z] / clearspawnpoint [player]",
  "  spreadplayers <x z> <spreadDistance> <maxRange> <targets>",
  "  stopsound <player> [sound]",
  "  structure save <name> <from> <to> [includeEntities] [disk|memory]",
  "  structure load <name> <to> [rotation] [mirror] [animation] [seed]",
  "  summon <entity> [x y z] [yRot xRot] [event] [name]",
  "  tag <target> add|remove|list [name]",
  "  teleport <target> <x y z> [yRot xRot] / teleport <target> <destination>",
  "  teleport <target> <x y z> facing <x y z>",
  "  tellraw <target> {\"rawtext\":[{\"text\":\"...\"}]}",
  "  testfor <target> / testforblock <x y z> <block> [data]",
  "  tickingarea add|remove|remove_all|list|preload <...>",
  "  time set day|night|noon|midnight|sunrise|sunset|<n>",
  "  time add <n> / time query day|daytime|gametime",
  "  title <player> title|subtitle|actionbar|clear|reset|times <text|n>",
  "  titleraw <player> title|subtitle|actionbar {\"rawtext\":[...]}",
  "  toggledownfall",
  "  weather clear|rain|thunder [seconds]",
  "  xp <amount> <player> for points, xp <amount>L <player> for levels",
  "",
  "These exist but are for running a server. Only use one if it is asked for:",
  "  allowlist, changesetting, dedicatedwsserver, deop, kick, list, op, ops,",
  "  permission, reload, reloadconfig, save, setmaxplayers, stop, transfer,",
  "  whitelist, worldbuilder, wb, immutableworld, connect, wsserver",
  "",
  "And these are for creators and tests, not for play:",
  "  aimassist, controlscheme, editor-allowlist, gametest, gametips, project,",
  "  remove, script, scriptevent, sendshowstoreoffer, set_movement_authority,",
  "  reloadpacketlimitconfig, party, hud",
